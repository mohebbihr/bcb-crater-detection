...
cnn_classifier.add(Dropout(0.3)) # quite aggresive dropout, maybe reduce
...
cnn_classifier.save_weights('cnn_models/center_train_weights_dropout.h5')
cnn_classifier.save('cnn_models/center_train_model_dropout.h5')
...
for i in range(0, len(files) - 1):
    X_eval.append(transform_image('./crater_data/challenging_test_set/crater/' + files[i + 1], size))
    y_eval.append(1)
...
for i in range(0, len(files) - 1):
    X_eval.append(transform_image('./crater_data/challenging_test_set/non-crater/' + files[i + 1], size))
    y_eval.append(0)
...
for plot_index, bad_index in enumerate(misclassified_indices[0:5]):
    ...
    plt.title(' Predicted: {}, Actual: {} '.format(labels_index[cnn_pred[bad_index][0]], 
                                                 labels_index[y_eval[bad_index]]), fontsize = 18)
plt.savefig('results/cnn/lime_mis_classified.png', dpi=400)
